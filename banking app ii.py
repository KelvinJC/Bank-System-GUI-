...
def finish_sign_up():
    ...
    if account_number in all_accounts:
        ...
        new_file.write('-0') # This gave me so much trouble when the placeholder was just 0. This way the replace function in deposit function does not replace any other zeros in the user data
        new_file.close()
...
def sign_up():
    ...
    # Sign up Screen
    sign_up_screen = Toplevel(master)
    sign_up_screen.title('Sign Up')
    sign_up_screen.resizable(width= False, height = False)

    # Labels
    Label(sign_up_screen, text="Please enter your details below to sign up", font=('Calibri', 12)).grid(row=0,sticky=N, pady=10)
    Label(sign_up_screen, text="Name", font=('Calibri', 12)).grid(row=1,sticky=W)
    Label(sign_up_screen, text="Email", font=('Calibri', 12)).grid(row=2,sticky=W)
    Label(sign_up_screen, text="Phone", font=('Calibri', 12)).grid(row=3,sticky=W)
    Label(sign_up_screen, text="Age", font=('Calibri', 12)).grid(row=4,sticky=W)
    Label(sign_up_screen, text="Gender", font=('Calibri', 12)).grid(row=5,sticky=W)
    Label(sign_up_screen, text="Address", font=('Calibri', 12)).grid(row=6,sticky=W)
    Label(sign_up_screen, text="Password", font=('Calibri', 12)).grid(row=7,sticky=W)

    notif = Label(sign_up_screen, font=('Calibri', 12))
    notif.grid(row=9,sticky=N, pady=10)
    account_notif = Label(sign_up_screen, font=('Calibri', 12, 'bold italic'))
    account_notif.grid(row=10,sticky=N, pady=10)

    # Entries
    Entry(sign_up_screen, textvariable=temp_name).grid(row=1, column=0)
    Entry(sign_up_screen, textvariable=temp_email).grid(row=2, column=0)
    Entry(sign_up_screen, textvariable=temp_phone).grid(row=3, column=0)
    Entry(sign_up_screen, textvariable=temp_age).grid(row=4, column=0)
    Entry(sign_up_screen, textvariable=temp_gender).grid(row=5, column=0)
    Entry(sign_up_screen, textvariable=temp_address).grid(row=6, column=0)
    Entry(sign_up_screen, textvariable=temp_password, show='*').grid(row=7, column=0)

    # Buttons
    Button(sign_up_screen, text='Sign Up', command= finish_sign_up, font=('Calibri', 12)).grid(row=8, sticky=N, pady=10)
...
def finish_deposit():
    ...
    # to handle the event of user entering a string as deposit
        ...
        with open(login_number, 'r') as file:
            file_data = file.read()

        details = file_data.split('\n')
        current_balance = details[8]
        updated_balance = current_balance
        updated_balance = float(updated_balance) + float(amount.get()) # inputs are converted to float before addition. In next line, output converted back to a string
        file_data1       = file_data.replace(current_balance, str(updated_balance)) # spacing the assignment operator just for better code readability

        with open(login_number, 'w') as file:
        ...
        #details = file_data1.split('\n')
        #current_balance = details[8:9]
        #updated_balance = current_balance         # seems redundant
        #updated_balance = float(updated_balance) + float(amount.get()) # inputs are converted to float before addition. In next line, output converted back to a string
        #file_data       = file_data1.replace(current_balance, str(updated_balance)) # spacing the assignment operator just for better code readability
...
def finish_withdraw():
    ...
    # to handle the event of user entering a string as deposit
        ...
        file = open(login_number, 'rw+')   # file is opened in r+ mode to accomodate future write operation
        file_data = file.read()
        details = file_data.split('\n')
        current_balance = details[8]
...
img = img.resize((323,576))

# there is the need to specify the master tk instance since ImageTK is a second instance of tkinter
bg = ImageTk.PhotoImage(img, master=master)

# Define canvas
my_canvas = Canvas(master, width=323, height=576)
...
# Add label
my_canvas.create_text(100, 110, text = 'Welcome \nto \nthe Future', font=("Helvetica", 30, 'bold italic'), fill='White')

# Add buttons
button1 = Button(master, text = 'Sign up', width = 10, command=sign_up)
button2 = Button(master, text = 'Log In', width = 10, command=login)


button1_window = my_canvas.create_window(10, 235, anchor= 'nw', window=button1)
button2_window = my_canvas.create_window(120, 235, anchor= 'nw', window=button2)
